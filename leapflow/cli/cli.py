...
async def _async_main(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if cmd == "interactive":
            ...
            if resume_id:
                from leapflow.cli.commands.learn import cmd_learn_resume
                return await cmd_learn_resume(ctx, resume_id, getattr(args, "timeout", None))
            from leapflow.cli.commands.learn import cmd_learn
            return await cmd_learn(
...
def main(argv: list[str] | None = None) -> int:
    ...
    # leap learn
    learn_parser = subparsers.add_parser("learn", parents=[common], help="Interactive learning mode")
    ...
    learn_parser.add_argument("--resume", metavar="ID", help="Resume a previous learning session")
    ...
    known_commands = {"learn", "run", "skills", "relearn", "host"}
    ...
    # Resolve learn goal from positional, --prompt, or --goal
